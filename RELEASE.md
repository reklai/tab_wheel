...
- Ordered the mouse-action dropdowns alphabetically by their label (Browser new tab, Close current tab, Drag current tab, Duplicate tab, Go back, Go forward, Most recent tab, Mute / unmute tab, Off, Open settings). This is display order only: defaults are unchanged, no migration runs, and every saved mapping is preserved exactly, so no existing user is affected.
