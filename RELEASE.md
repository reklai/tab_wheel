...
- Slowed Drag current tab from one slot per 56 px to one slot per 96 px, so the tab tracks the pointer more deliberately and is less prone to overshooting. Nothing else about the drag changed.
