...
- Made the popup's Refresh extension button reconnect TabWheel on every open web tab, matching the settings page. It previously only reinjected the current tab, so a tab in another window could stay disconnected after an update until you visited it.
- Added a Drag speed slider to the popup and settings page so you can set how far to drag per tab, from slower to faster (about 96 px per slot at the default). Existing users start at the default, unchanged.
...
- Added a quiet rating star beside the settings gear in the popup header and beside the close button in the settings page header, on the Chrome build only. It is a filled amber star linking to the store's reviews page, with a tooltip and no prompt, timer, storage, or dismiss logic; the Firefox build renders nothing there until a Firefox listing exists.
