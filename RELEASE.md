...
- Made Drag current tab sticky so it no longer overshoots. A settled slot now holds until the pointer clearly reverses (one step plus a small margin), so a hand that over-travels into the next slot or jitters near a boundary no longer bounces the tab back and forth. Forward travel is unchanged at one slot per 56 px, and a deliberate reversal still steps back.
