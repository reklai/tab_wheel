...
- Made Drag current tab slower and stickier so it no longer overshoots. Each slot now takes a more deliberate 100 px of travel (was 56 px), and a settled slot holds until the pointer clearly reverses, so a hand that over-travels or jitters near a boundary no longer bounces the tab. A deliberate reversal still steps back. A settled slot now holds until the pointer clearly reverses (one step plus a small margin), so a hand that over-travels into the next slot or jitters near a boundary no longer bounces the tab back and forth. Forward travel is unchanged at one slot per 56 px, and a deliberate reversal still steps back.
