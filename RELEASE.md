...
- Slowed Drag current tab a little, from one slot per 56 px to one slot per 80 px, so the tab tracks the pointer more deliberately and is less prone to overshooting. Nothing else about the drag changed.
